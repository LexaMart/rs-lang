  ...
  switch (action.type) {
    ...
    case ACTION_CONST.SET_ACTIVE_CARD: {
      const randomValue = Math.floor(
        Math.random() * Math.floor(state.remainWordsData.length)
      );
    ...
    case ACTION_CONST.SET_CARDS_FOR_SELECTION: {
      ...
      if (state.activeCard) {
        arrayOfCardsForSelect.filter((card) => {
          return card.id !== state.activeCard.id;
        });
      }
      //TODO
      const length = 3;
      ...
      for (let i = 0; i < length; i++) {
        let index = Math.floor(
          Math.random() * Math.floor(arrayOfCardsForSelect.length - 1)
        );
