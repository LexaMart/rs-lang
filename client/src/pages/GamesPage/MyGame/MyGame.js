...
import {
  setMyGameLearnedWords,
  setMyGameIncorrectAnswers,
  getStatistic,
...
export const MyGame = () => {
  ...
  useEffect(
      ...
      if (isGameStarted) {
        ...
        const cards = await request(
          `${urls.API}/words?group=${
            currentPage === 'main' ? currentWordsGroup : levelInputValue - 1
          }&page=${
            currentPage === 'main' ? currentWordsPage : pageInputValue - 1
          }`,
          'GET'
        );
        for (let i = 0; i <= 3; i++) {
    ...
    }, [isGameStarted, gameCards, request, levelInputValue, pageInputValue]),
    [isGameStarted, pageInputValue, levelInputValue, randomInt]
  );
  ...
  return (
    <div className="our-game-container">
      {!isGameStarted && !isGameLost && (
        <p className="rules">
          {language === LANGUAGE_CONFIG.foreign
            ? WORDS_CONFIG.MYGAMES_PAGE_RULES.foreign
            : WORDS_CONFIG.MYGAMES_PAGE_RULES.native}
        </p>
      )}
      {isGameLost && !isGameStarted && (
        <div className="lost-screen">
          {language === LANGUAGE_CONFIG.foreign
            ? WORDS_CONFIG.MYGAMES_PAGE_LOST.foreign
            : WORDS_CONFIG.MYGAMES_PAGE_LOST.native}{' '}
          <b>{score} </b>{' '}
        </div>
      )}
      ...
      {!isGameStarted && (
        <button
          className="btn start-our waves-effect waves-light red"
          type="button"
          onClick={startGame}
        >
          {!isGameLost
      ...
      {isGameStarted && winingCard && (
        <div className="word-to-guess-block">
          ...
          <div className="lives-container">
            <div>
              <i className="material-icons">favorite </i> x {lives.length + 1}
            </div>
          </div>
          <div className="image-handler-our">
            {gameCards.map((el, key) => (
              <ImageCard choiceHandler={choiceHandler} key={key} el={el} />
            ))}
          </div>
        </div>
      )}
